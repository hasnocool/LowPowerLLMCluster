...
from lowpower_llm_cluster.history_compat import _merge_active_promotion
from lowpower_llm_cluster.live_discoveries import LIVE_DISCOVERIES_HTML
from lowpower_llm_cluster.promotion_state import build_promotion_snapshot, filter_promotion_items
...
def record(source_id: str, *, observed_at: str, title: str | None = None) -> dict:
    return {
        ...
        "title": title or f"AI accelerator {source_id}",
...
def test_snapshot_exposes_all_four_promotion_states(tmp_path: Path) -> None:
    ...
    held = record("held", observed_at="2026-08-13T20:00:00Z")
    held["manufacturer"] = ""
    ...
    write_json(report, {
        "generated_at": "2026-08-13T20:05:00Z",
        "held": [{"source": "vendor", "source_id": "held", "reasons": ["missing_manufacturer"]}],
    })
...
def test_active_merge_does_not_count_stale_promotion_records(tmp_path: Path) -> None:
    current = record("current", observed_at="2026-08-13T20:00:00Z")
    stale = record("stale", observed_at="2026-08-13T20:00:00Z")
    discovery = tmp_path / "discovery.json"
    report = tmp_path / "promotion.json"
    catalog = tmp_path / "catalog.json"
    write_json(discovery, {"observations": [current, stale]})
    write_json(report, {"generated_at": "2026-08-13T20:05:00Z", "held": []})
    write_json(catalog, {"parts": [canonical_part(stale)]})
    snapshot = build_promotion_snapshot(discovery_path=discovery, report_path=report, catalog_path=catalog)
    live = {"sources": ["vendor"], "items": [{
        "source": "vendor", "source_id": "current", "title": "AI accelerator current",
        "listing_url": "https://vendor.example/products/current", "last_seen_at": "2026-08-13T20:06:00Z",
        "price": 99.0, "currency": "USD", "in_stock": True,
    }]}
    merged = _merge_active_promotion(live, snapshot)
    assert merged["total"] == 1
    assert merged["counts"]["promotion_ready"] == 1
    assert merged["counts"]["canonical"] == 0

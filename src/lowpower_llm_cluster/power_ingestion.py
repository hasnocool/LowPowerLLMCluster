...
def benchmark_power_observations(performance: dict[str, Any], catalog: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    # Direct measured watt fields, if current/future benchmark records expose them.
    for row in records:
    ...
    # W = (tokens/s) / (tokens/J), but only for records with the exact same benchmark signature.
...
def refresh_power_evidence(
    *,
    ...
    """Append newly available catalog and benchmark power observations to the learned store."""
    catalog = catalog or load_catalog()
    ...
    candidates = [*catalog_power_observations(catalog), *benchmark_power_observations(performance, catalog)]

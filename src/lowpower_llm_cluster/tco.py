...
def load_tco_scenarios(path: Path | None = None) -> dict[str, Any]:
    ...
    return _load(target, {"schema_version": 2, "component_costs_cad": {}, "energy_scenarios": {}})
...
def complete_node_power(part: dict[str, Any], scenarios: dict[str, Any]) -> dict[str, Any]:
    ...
    if target_w is None:
        return {
            ...
            "basis": "unknown",
            "confidence": "unknown",
            "warnings": ["No usable power evidence is available for this product."],
        }
...
def break_even_analysis(
    ...
    *,
    ...
    """Solve transparent break-even thresholds between two complete-node options."""
    scenarios = scenarios or load_tco_scenarios()
    scenario = _scenario(scenarios, scenario_name)
    tco_a = evaluate_tco(part_a, price_a_cad, scenario_name=scenario_name, scenarios=scenarios)
    tco_b = evaluate_tco(part_b, price_b_cad, scenario_name=scenario_name, scenarios=scenarios)
    ...
    power_a = tco_a.get("power") or {}
    power_b = tco_b.get("power") or {}
    load_a = power_a.get("load_w")
    load_b = power_b.get("load_w")
    idle_a = power_a.get("idle_w")
    idle_b = power_b.get("idle_w")
    years = float(scenario.get("years", 3))
    days = float(scenario.get("days_per_year", 365))
    load_h = float(scenario.get("load_hours_per_day", 0))
    idle_h = float(scenario.get("idle_hours_per_day", 0))
    rate = float(scenario.get("electricity_cad_per_kwh", 0))
    ...
    if None not in (load_a, load_b, idle_a, idle_b):
        ...
        if abs(period_delta_kwh) > 1e-9:
            ...
            if candidate >= 0:
                rate_break_even = round(candidate, 4)

        total_daily_hours = load_h + idle_h
        if total_daily_hours > 0 and rate > 0 and years > 0 and days > 0:
            delta_load = float(load_a) - float(load_b)
            delta_idle = float(idle_a) - float(idle_b)
            ...
            if abs(denominator) > 1e-9:
                ...
                if 0 <= candidate_h <= total_daily_hours:
                    load_hours_break_even = round(candidate_h, 2)

    total_a = tco_a.get("total_cost_of_ownership_cad")
    total_b = tco_b.get("total_cost_of_ownership_cad")
    cheaper = None
    if total_a is not None and total_b is not None:
        cheaper = "a" if float(total_a) < float(total_b) else "b" if float(total_b) < float(total_a) else "equal"

    return {
        "scenario": scenario_name,
        "a": {"id": part_a.get("id"), "name": part_a.get("name"), "price_cad": price_a_cad, "tco": tco_a},
        "b": {"id": part_b.get("id"), "name": part_b.get("name"), "price_cad": price_b_cad, "tco": tco_b},
        "currently_cheaper": cheaper,
        "price_a_break_even_cad": round(price_a_break_even, 2),
        "price_b_break_even_cad": round(price_b_break_even, 2),
        "electricity_rate_break_even_cad_per_kwh": rate_break_even,
        "load_hours_per_day_break_even": load_hours_break_even,
        "notes": [
            "Product-price thresholds hold every other selected scenario assumption constant.",
            "Electricity-rate break-even is only reported when the two options have different modeled energy use and yields a non-negative rate.",
            "Load-hours break-even keeps the scenario's total daily powered-on hours constant and shifts hours between idle and load.",
            "Power-derived thresholds remain planning estimates when either option lacks measured complete-node input power.",
        ],
    }


def apply_tco_to_summary(summary: dict[str, Any], *, scenario_name: str = "mixed-3yr", scenarios: dict[str, Any] | None = None) -> dict[str, Any]:
    scenarios = scenarios or load_tco_scenarios()
    parts = {str(part["id"]): part for part in load_catalog()["parts"]}
    rows = list(summary.get("recommendations", []))
    for row in rows:
        part = parts.get(str(row.get("id")))
        row["tco"] = evaluate_tco(part, row.get("current_cad"), scenario_name=scenario_name, scenarios=scenarios) if part else None
    totals = sorted(
        float((row.get("tco") or {}).get("total_cost_of_ownership_cad"))
        for row in rows
        if (row.get("tco") or {}).get("total_cost_of_ownership_cad") is not None
    )
    for row in rows:
        tco = row.get("tco") or {}
        total = tco.get("total_cost_of_ownership_cad")
        old_score = float(row.get("deal_score") or 0.0)
        percentile = None
        if total is not None and totals:
            ...
            acquisition = tco.get("complete_node_acquisition_cad")
            product = tco.get("product_price_cad")
            period = (tco.get("operating") or {}).get("period_cad")
            if acquisition and product and float(acquisition) > float(product) * 1.5:
                adjusted -= 5.0
            if acquisition and period and float(period) > float(acquisition) * 0.5:
                adjusted -= 5.0
        else:
            adjusted = min(old_score, 59.0)
        adjusted = round(max(0.0, min(100.0, adjusted)), 1)
        row["pre_tco_deal_score"] = old_score
        row["deal_score"] = adjusted
        row["tco_percentile"] = round(percentile, 3) if percentile is not None else None
        original = str(row.get("recommendation"))
        if original == "Experimental":
            recommendation = original
        elif total is None:
            recommendation = "Watch" if original == "Buy" else original
        elif adjusted >= 72 and original in {"Buy", "Watch"}:
            recommendation = "Buy"
        elif adjusted >= 45:
        ...
        if infra.get("total_cad", 0):
            row.setdefault("reasons", []).append(f"complete-node infrastructure assumption CA${float(infra['total_cad']):,.0f}")
        if total is not None:
            row.setdefault("reasons", []).append(f"{scenario_name} TCO CA${float(total):,.0f}")
    order = {"Buy": 0, "Watch": 1, "Experimental": 2, "Ignore": 3}
    rows.sort(key=lambda row: (order.get(str(row.get("recommendation")), 9), -float(row.get("deal_score") or 0.0), str(row.get("name") or "")))
    summary["recommendations"] = rows
    summary["counts"] = {name: sum(1 for row in rows if row.get("recommendation") == name) for name in ("Buy", "Watch", "Experimental", "Ignore")}
    summary["tco_scenario"] = scenario_name
    summary.setdefault("method_notes", []).extend([
        "Decision scores are re-ranked using complete-node acquisition and operating cost after the market/model-fit score is computed.",
        "Discrete-GPU new-build infrastructure explicitly includes CPU/host, motherboard, RAM, storage, PSU, PCIe integration, cooling and chassis.",
        "Infrastructure and electricity values are editable planning assumptions unless a component has separately sourced market evidence.",
        "Board TGP/TBP is converted only into a low-confidence complete-node planning estimate by explicitly adding host and overhead assumptions; it is never relabeled as measured wall power.",
    ])
    return summary
...
def render_tco_report(summary: dict[str, Any], *, limit: int = 30) -> str:
    lines = [
        ...
        "",
        f"Scenario: **{summary.get('tco_scenario', 'mixed-3yr')}**",
        "",
        "Product price is separated from required infrastructure and operating electricity. Infrastructure/electricity defaults are planning assumptions and should be replaced with local sourced values before purchase.",
        "",
        "Decision | Score | Product | Infrastructure | Complete node | Operating | TCO | Power basis | Candidate",
        "--- | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---",
    ]
    for row in list(summary.get("recommendations", []))[:limit]:
        ...
        power = tco.get("power") or {}
        def money(value: Any) -> str:
            return f"CA${float(value):,.0f}" if value is not None else "—"
        lines.append(
            f"{row.get('recommendation')} | {float(row.get('deal_score') or 0):.1f} | {money(tco.get('product_price_cad'))} | {money(infra.get('total_cad'))} | {money(tco.get('complete_node_acquisition_cad'))} | {money(operating.get('period_cad'))} | {money(tco.get('total_cost_of_ownership_cad'))} | {power.get('basis', 'unknown')} | {row.get('name')}"
        )
    lines.extend([
        "",
        "## Reading this report",
        "",
        "- Product = current sourced/listed product cost when available.",
        "- Infrastructure = CPU/host + motherboard + RAM + storage + PSU + PCIe/OCuLink/riser + cooling + chassis planning assumptions required by the deployment profile.",
        "- Complete node = product + infrastructure.",
        "- Operating = electricity over the selected usage/rate scenario.",
        "- TCO = complete-node acquisition + scenario operating cost.",
        "- Low-confidence power estimates remain planning values until complete-node input is actually measured.",
        "",
    ])
    return "\n".join(lines)

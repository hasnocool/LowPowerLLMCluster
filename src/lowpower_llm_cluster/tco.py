...
def infrastructure_cost(part: dict[str, Any], scenarios: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"profile": requirements["profile"], "components": lines, "total_cad": round(total, 2), "complete": all(line["cad"] is not None for line in lines), "warnings": requirements["warnings"]}
...
def operating_cost(power: dict[str, Any], scenario: dict[str, Any]) -> dict[str, Any]:
    idle_w = power.get("idle_w"); load_w = power.get("load_w")
...
def evaluate_tco(part: dict[str, Any], product_price_cad: float | None, *, scenario_name: str = "mixed-3yr", scenarios: dict[str, Any] | None = None) -> dict[str, Any]:
    scenarios = scenarios or load_tco_scenarios(); infra = infrastructure_cost(part, scenarios); power = complete_node_power(part, scenarios)
    energy_scenarios = scenarios.get("energy_scenarios") or {}; scenario = energy_scenarios.get(scenario_name) or next(iter(energy_scenarios.values()), {}); energy = operating_cost(power, scenario)
    ...
    return {"scenario": scenario_name, "product_price_cad": round(float(product_price_cad), 2) if product_price_cad is not None else None, "infrastructure": infra, "complete_node_acquisition_cad": acquisition, "product_share_of_acquisition": product_share, "power": power, "operating": energy, "total_cost_of_ownership_cad": total, "basis": "live/sourced product price plus editable infrastructure and electricity planning assumptions", "warnings": list(dict.fromkeys([*infra.get("warnings", []), *power.get("warnings", [])]))}
...
def apply_tco_to_summary(summary: dict[str, Any], *, scenario_name: str = "mixed-3yr", scenarios: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        part = parts.get(str(row.get("id"))); row["tco"] = evaluate_tco(part, row.get("current_cad"), scenario_name=scenario_name, scenarios=scenarios) if part else None
    ...
    for row in rows:
        ...
        if total is not None and totals:
            percentile = sum(1 for value in totals if value <= float(total)) / len(totals); adjusted = (old_score * 0.75) + ((1.0 - percentile) * 25.0)
        ...
        if infra.get("total_cad", 0): row.setdefault("reasons", []).append(f"complete-node infrastructure assumption CA${float(infra['total_cad']):,.0f}")
    ...
    summary["recommendations"] = rows; summary["counts"] = {name: sum(1 for row in rows if row.get("recommendation") == name) for name in ("Buy", "Watch", "Experimental", "Ignore")}; summary["tco_scenario"] = scenario_name
    summary.setdefault("method_notes", []).extend(["Decision scores are re-ranked using complete-node acquisition and operating cost after the market/model-fit score is computed.", "Infrastructure and electricity values are editable planning assumptions unless a component has separately sourced market evidence.", "Board TGP/TBP is converted only into a low-confidence complete-node planning estimate by explicitly adding host and overhead assumptions; it is never relabeled as measured wall power."])
    return summary


def render_tco_report(summary: dict[str, Any], *, limit: int = 30) -> str:
    lines = ["# Complete-Node Total Cost of Ownership", "", f"Scenario: **{summary.get('tco_scenario', 'mixed-3yr')}**", "", "Product price is separated from required infrastructure and operating electricity. Infrastructure/electricity defaults are planning assumptions and should be replaced with local sourced values before purchase.", "", "Decision | Score | Product | Infrastructure | Complete node | Operating | TCO | Power basis | Candidate", "--- | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---"]
    for row in list(summary.get("recommendations", []))[:limit]:
        tco = row.get("tco") or {}; infra = tco.get("infrastructure") or {}; operating = tco.get("operating") or {}; power = tco.get("power") or {}
        def money(value: Any) -> str: return f"CA${float(value):,.0f}" if value is not None else "—"
        lines.append(f"{row.get('recommendation')} | {float(row.get('deal_score') or 0):.1f} | {money(tco.get('product_price_cad'))} | {money(infra.get('total_cad'))} | {money(tco.get('complete_node_acquisition_cad'))} | {money(operating.get('period_cad'))} | {money(tco.get('total_cost_of_ownership_cad'))} | {power.get('basis', 'unknown')} | {row.get('name')}")
    lines.extend(["", "## Reading this report", "", "- Product = current sourced/listed product cost when available.", "- Infrastructure = host/RAM/storage/PSU/PCIe/cooling/chassis planning assumptions required by the deployment profile.", "- Complete node = product + infrastructure.", "- Operating = electricity over the selected usage/rate scenario.", "- TCO = complete-node acquisition + scenario operating cost.", "- Low-confidence power estimates remain planning values until complete-node input is actually measured.", ""]); return "\n".join(lines)

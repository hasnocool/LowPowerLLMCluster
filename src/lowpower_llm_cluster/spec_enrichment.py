...
from .structured_specs import ingest_structured_manufacturer_document
...
def associate_spec_source(component: str, listing: dict[str, Any], config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    sku = _norm(listing.get("sku"))
    haystack = f"{title} {sku}".strip()
    matches: list[tuple[int, dict[str, Any]]] = []
    for association in config.get("associations", []):
        if association.get("component") != component:
            continue
        exact_skus = [_norm(value) for value in association.get("exact_skus", []) if value]
        match_terms = [_norm(value) for value in association.get("match_terms", []) if value]
        exact = bool(sku and exact_skus and sku in exact_skus)
        term_hits = sum(1 for term in match_terms if term in haystack)
        if exact or term_hits:
            matches.append((100 if exact else term_hits, association))
    if not matches:
        return None
...
def _cast(value: str, kind: str) -> Any:
    ...
    if kind == "float":
        return float(value)
    if kind == "bool":
        return value.strip().casefold() in {"1", "true", "yes", "supported"}
    return value.strip()
...
def extract_spec_fields(text: str, association: dict[str, Any], source_url: str, observed_at: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for field, rule in (association.get("fields") or {}).items():
        ...
        if "regex" in rule:
            ...
            if match:
                value = _cast(match.group(int(rule.get("group", 1))), str(rule.get("cast", "str")))
                extraction = "manufacturer_page_regex"
        elif "constant" in rule:
            value = rule["constant"]
            extraction = "verified_manufacturer_page_constant"
        elif "contains" in rule:
            found = [out for needle, out in rule["contains"].items() if _norm(needle) in _norm(normalized)]
            if found:
                value = found if rule.get("many", False) else found[0]
                extraction = "manufacturer_page_contains"
        if value is None:
            continue
        facts[field] = value
        evidence[field] = {
            ...
            "confidence": "exact" if association.get("exact_skus") else "high",
...
def _auto_fact_evidence(facts: dict[str, Any], source_url: str, observed_at: str, association_id: str, identity_score: float | None) -> dict[str, Any]:
    ...
    return {
        field: {
            ...
            "source_type": "manufacturer_spec",
            "observed_at": observed_at,
            "extraction": "automatic_manufacturer_page_parser",
...
def extract_automatic_spec_fields(component: str, text: str, source_url: str, observed_at: str, association_id: str, identity_score: float | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    """Conservative flattened-text fallback used only after structured document ingestion."""
    raw = " ".join(text.split())
    lower = raw.casefold()
    facts: dict[str, Any] = {}

    if component in {"cpu_host", "motherboard", "cooling"}:
        sockets = [value for value in ("AM4", "AM5", "LGA1700", "LGA1851") if value.casefold() in lower]
    ...
    if component == "cpu_host":
        ...
        if generation:
            facts["pcie_generation"] = generation
        ...
        if "atx" in lower:
            forms: list[str] = []
            if re.search(r"\b(?:micro[- ]?atx|matx|m-atx)\b", lower):
                forms.append("mATX")
            if re.search(r"\batx\b", lower) and not forms:
                forms.append("ATX")
            if forms:
                facts["form_factors"] = forms
        if re.search(r"PCI(?:e| Express)[^\n]{0,40}x16", raw, re.IGNORECASE):
        ...
        if m2_hits:
            ...
            facts["m2_slots"] = min(m2_hits, 8)
        share = re.search(r"([^.!?]{0,120}(?:unavailable|disabled|shares? bandwidth|share[^.!?]{0,30}lanes?)[^.!?]{0,120})", raw, re.IGNORECASE)
        if share:
            facts["lane_sharing_note"] = share.group(1).strip()
        ...
        if "atx 3.1" in lower:
            facts["atx_standard"] = "ATX 3.1"
        elif "atx 3.0" in lower:
            facts["atx_standard"] = "ATX 3.0"
        connectors: list[str] = []
        if any(token in lower for token in ("12v-2x6", "12v 2x6", "12vhpwr")):
            connectors.append("12V-2x6")
        if any(token in lower for token in ("6+2", "6 + 2", "8-pin pcie", "8 pin pcie")):
        ...
        length = _first_int([r"(?:Length|Card Length|Graphics Card Dimensions)[^0-9]{0,30}(\d{3})\s*mm", r"(\d{3})\s*mm\s*[x×]\s*\d{2,3}\s*mm"], raw, minimum=100, maximum=600)
        if length:
        ...
        if psu:
            facts["minimum_psu_w"] = psu
        connectors: list[str] = []
        if any(token in lower for token in ("12v-2x6", "12v 2x6", "12vhpwr")):
            connectors.append("12V-2x6")
        if any(token in lower for token in ("8-pin", "8 pin", "8pin")):
            connectors.append("8-pin")
        if connectors:
            facts["power_connectors"] = sorted(set(connectors))
        lane = re.search(r"PCI(?:e| Express)\s*(?:Gen\s*)?(\d)(?:\.0)?\s*x(\d{1,2})", raw, re.IGNORECASE)
        if lane:
            facts["pcie_generation"] = int(lane.group(1))
            facts["minimum_pcie_lanes"] = int(lane.group(2))
            facts["pcie_slot"] = "x16" if int(lane.group(2)) <= 16 else f"x{lane.group(2)}"
        if "resizable bar" in lower or "rebar" in lower:
            facts["resizable_bar_required"] = "required" in lower or "must" in lower
...
def _fill_missing(base: dict[str, Any], base_evidence: dict[str, Any], fallback: dict[str, Any], fallback_evidence: dict[str, Any]) -> None:
    for field, value in fallback.items():
        if field in base:
            continue
        base[field] = value
        if field in fallback_evidence:
            base_evidence[field] = fallback_evidence[field]
...
async def enrich_candidate(component: str, candidate: dict[str, Any], config: dict[str, Any], *, client: httpx.AsyncClient) -> dict[str, Any]:
    ...
    association = associate_spec_source(component, listing, config)
    automatic = False
    ...
    if association is None:
        candidate["spec_enrichment"] = {"status": "unassociated", "fields_added": 0}
        return candidate

    url = str(association.get("source_url") or "")
    observed_at = _now()
    page_html, fetch_error = await _fetch_verified_page(association, client)
    if page_html is None:
        candidate["spec_enrichment"] = {
            "status": "fetch_failed" if fetch_error != "invalid_source_url" else "invalid_source_url",
            "fields_added": 0,
            "association_id": association.get("id") or association.get("mpn"),
            "source_url": url,
            "error": fetch_error,
        }
        return candidate
    ...
    if not automatic:
        ...
        if verify_terms and not any(term in _norm(text) for term in verify_terms):
            candidate["spec_enrichment"] = {"status": "identity_not_verified", "fields_added": 0, "association_id": association.get("id"), "source_url": url}
            return candidate
    ...
    target_cpu = configuration.get("cpu") or configuration.get("processor")
    structured_facts, structured_evidence, structured_stats = await ingest_structured_manufacturer_document(
        component,
        page_html,
        url,
        observed_at,
    ...
    merged = dict(candidate.get("compatibility_facts") or {})
    merged.update(facts)
    candidate["compatibility_facts"] = merged
    provenance = dict(candidate.get("compatibility_fact_provenance") or {})
    provenance.update(evidence)
    candidate["compatibility_fact_provenance"] = provenance
    candidate["spec_enrichment"] = {
        "status": "enriched" if facts else "verified_no_fields_extracted",
        "fields_added": len(facts),
        "association_id": association_id,
        "manufacturer": manufacturer,
        "source_url": url,
        "observed_at": observed_at,
        "association_origin": "automatic" if automatic else "curated",
        "identity_score": identity_score,
        "cache_hit": association.get("cache_hit") if automatic else None,
        "structured_document": structured_stats,
        "structured_fields_added": len(structured_facts),
    }
    return candidate


async def enrich_market_candidate(component: str, candidate: dict[str, Any], *, config_path: Path | None = None) -> dict[str, Any]:
    config = load_spec_enrichment_config(config_path)
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers={"User-Agent": USER_AGENT}) as client:
        for component, component_row in components.items():
            for candidate in component_row.get("candidates") or []:
                await enrich_candidate(component, candidate, config, client=client)
                enrichment = candidate.get("spec_enrichment") or {}
                if enrichment.get("status") == "enriched":
                    evidence_rows.append({
                        "component": component,
                        "source_id": (candidate.get("listing") or {}).get("source_id"),
                        "title": (candidate.get("listing") or {}).get("title"),
                        "spec_enrichment": enrichment,
                        "field_evidence": candidate.get("compatibility_fact_provenance") or {},
                    })
    target = evidence_path or project_root() / "data" / "market" / "spec-evidence.json"
    payload = {"schema_version": 3, "generated_at": _now(), "records": evidence_rows}

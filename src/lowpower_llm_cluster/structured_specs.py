...
def _support_status(value: str, bios: str) -> str:
    normalized = _norm(value)
    if any(term in normalized for term in ("unsupported", "not supported", "not support", "no support")):
        return "unsupported"
    if normalized in {"no", "n", "false"}:
        return "unsupported"
    if any(term in normalized for term in ("supported", "support", "validated", "yes")):
        return "supported"
    if bios and _norm(bios) not in {"-", "n/a", "na", "none", "not supported"}:
        return "supported"
    return "unknown"
...
def extract_cpu_support_rows(tables: list[list[list[str]]]) -> list[dict[str, Any]]:
    ...
    for table_index, table in enumerate(tables):
        ...
        status_idx = next((i for i, cell in enumerate(headers) if any(term in cell for term in ("support", "status", "validated"))), None)
        if cpu_idx is None or bios_idx is None:
            continue
        for row_index, row in enumerate(table[1:], start=1):
            if len(row) <= max(cpu_idx, bios_idx):
                continue
            cpu_model = row[cpu_idx].strip()
            bios = row[bios_idx].strip()
            if not cpu_model:
                continue
            status_text = row[status_idx].strip() if status_idx is not None and len(row) > status_idx else ""
            rows.append({
                "cpu_model": cpu_model,
                "minimum_bios_version": bios or None,
                "support_status": _support_status(status_text, bios),
...
def extract_cpu_support_matrix(tables: list[list[list[str]]], target_cpu: str | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        cpu_name = str(row.get("cpu_model") or "")
        if target and target not in _norm(cpu_name):
            continue
        return {
            "cpu_support_model": cpu_name,
            "minimum_bios_version": row.get("minimum_bios_version"),
            "cpu_support_status": row.get("support_status"),
        }
    return {}
...
async def ingest_structured_manufacturer_document(
    ...
    *,
    ...
    _merge(facts, evidence, jsonld, {
        "source_url": source_url,
        "source_type": "manufacturer_structured",
        "observed_at": observed_at,
        "extraction": "schema_org_additionalProperty",
    ...
    _merge(facts, evidence, table_facts, {
        "source_url": source_url,
        "source_type": "manufacturer_structured",
        "observed_at": observed_at,
        "extraction": "html_spec_table",
        "association_id": association_id,
        "identity_score": identity_score,
        "confidence": confidence,
    })
    ...
    if component == "motherboard":
        ...
        for row in support_rows:
            row.update({
                ...
                "observed_at": observed_at,
                "extraction": "cpu_bios_support_matrix",
                "association_id": association_id,
                "identity_score": identity_score,
                "confidence": confidence,
            })
        ...
        _merge(facts, evidence, support, {
            ...
            "association_id": association_id,
            "identity_score": identity_score,
            "confidence": "exact" if target_cpu else "high",
        })
    ...
    if client is not None:
        ...
        for pdf_url in links:
            ...
            _merge(facts, evidence, pdf_facts, {
                "source_url": pdf_url,

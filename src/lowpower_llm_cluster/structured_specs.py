...
class StructuredHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if lower == "script" and values.get("type", "").casefold() == "application/ld+json":
            self._json_capture = True; self._json_chunks = []
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if lower == "script" and self._json_capture:
            ...
            self._row.append(" ".join("".join(self._cell_chunks).split())); self._cell_chunks = None
        elif lower == "tr" and self._row is not None and self._table is not None:
            if any(cell for cell in self._row): self._table.append(self._row)
            self._row = None
            ...
            self.links.append({"href": self._href, "text": " ".join("".join(self._link_chunks).split())}); self._href = None; self._link_chunks = []
...
def _property_pairs(value: Any) -> list[tuple[str, str]]:
    rows = value if isinstance(value, list) else [value]; out: list[tuple[str, str]] = []
    for row in rows:
...
def extract_table_facts(component: str, tables: list[list[list[str]]]) -> tuple[dict[str, Any], list[dict[str, str]]]:
    pairs: list[tuple[str, str]] = []; support_rows: list[dict[str, str]] = []
    for table in tables:
        for row in table:
...
def extract_cpu_support_matrix(tables: list[list[list[str]]], target_cpu: str | None = None) -> dict[str, Any]:
    ...
    for table in tables:
        ...
        for row in table[1:]:
            ...
            if target and target not in _norm(cpu_name): continue
...
async def ingest_structured_manufacturer_document(component: str, html: str, source_url: str, observed_at: str, association_id: str, identity_score: float | None, *, client: httpx.AsyncClient | None = None, target_cpu: str | None = None, allowed_host: str | None = None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    parser = StructuredHTMLParser(); parser.feed(html); facts: dict[str, Any] = {}; evidence: dict[str, Any] = {}; stats: dict[str, Any] = {"jsonld_fields": 0, "table_fields": 0, "support_fields": 0, "pdf_fields": 0, "pdf_urls": []}
    confidence = "exact" if identity_score and identity_score >= 0.9 else "high"
    jsonld = extract_jsonld_facts(component, parser.jsonld)
    _merge(facts, evidence, jsonld, {"source_url": source_url, "source_type": "manufacturer_structured", "observed_at": observed_at, "extraction": "schema_org_additionalProperty", "association_id": association_id, "identity_score": identity_score, "confidence": confidence}); stats["jsonld_fields"] = len(jsonld)
    table_facts, _ = extract_table_facts(component, parser.tables)
    _merge(facts, evidence, table_facts, {"source_url": source_url, "source_type": "manufacturer_structured", "observed_at": observed_at, "extraction": "html_spec_table", "association_id": association_id, "identity_score": identity_score, "confidence": confidence}); stats["table_fields"] = len(table_facts)
    if component == "motherboard":
        support = extract_cpu_support_matrix(parser.tables, target_cpu=target_cpu)
        _merge(facts, evidence, support, {"source_url": source_url, "source_type": "manufacturer_support_table", "observed_at": observed_at, "extraction": "cpu_bios_support_matrix", "association_id": association_id, "identity_score": identity_score, "confidence": "exact" if target_cpu else "high"}); stats["support_fields"] = len(support)
    if client is not None:
        links = find_document_links(parser, source_url)[:3]; source_host = (urlparse(source_url).hostname or "").casefold(); allowed = (allowed_host or source_host).casefold()

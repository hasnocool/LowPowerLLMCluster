...
def infer_listing_facts(component: str, title: str, spec: dict[str, Any]) -> dict[str, Any]:
    """Infer only explicitly configured or text-supported compatibility facts.

    Variant facts are trusted only when one of that variant's match terms is present.
    Generic numeric facts are parsed conservatively from the listing title.
    """
    text = _norm(title)
    ...
    if component in {"host_ram_32gb", "system_ram_32gb"}:
        ...
        if any(term in text for term in ("12vhpwr", "12v 2x6", "12v2x6")):
            facts.setdefault("gpu_power_connectors", []).append("12V-2x6")
        if "8 pin" in text or "8pin" in text or "6+2" in text:
            facts.setdefault("gpu_power_connectors", []).append("8-pin")
...
def gpu_requirements(part: dict[str, Any]) -> dict[str, Any]:
    """Return explicit GPU host requirements from catalog metadata only."""
...
def evaluate_build_compatibility(build: dict[str, dict[str, Any]], gpu_part: dict[str, Any] | None = None) -> dict[str, Any]:
    """Evaluate cross-component constraints without treating missing facts as compatible facts."""
    failures: list[str] = []
    unknowns: list[str] = []
    ...
    if gpu_part:
        ...
        if board_slot is None:
            ...
            failures.append(f"pcie_gpu_slot: {board_slot} is not x16")
        min_psu = gpu.get("minimum_psu_w")
        psu_w = psu.get("wattage_w")
        ...
        gpu_length = gpu.get("gpu_length_mm")
        case_length = chassis.get("max_gpu_length_mm")
        ...
        gpu_slots = gpu.get("gpu_slots")
        case_slots = chassis.get("max_gpu_slots")
    ...
    case_cooler = chassis.get("max_cpu_cooler_height_mm")
    ...
    lane_req = int((gpu_requirements(gpu_part).get("minimum_pcie_lanes") if gpu_part else 0) or 0)
    ...
    confidence = "high" if status == "compatible" else "medium" if status == "provisionally_compatible" and len(unknowns) <= 2 else "low" if status == "provisionally_compatible" else "rejected"
    return {
        "status": status,
...
def load_builds(path: Path | None = None) -> dict[str, Any]:
    ...
    return _load(target, {"schema_version": 1, "builds": []})

...
def infer_listing_facts(component: str, title: str, spec: dict[str, Any]) -> dict[str, Any]:
    text = _norm(title); facts: dict[str, Any] = {"component": component}; matched_variant = None
    for variant in spec.get("variants", []):
        terms = [_norm(term) for term in variant.get("match_terms", [])]
        if terms and any(term in text for term in terms):
            facts.update(variant.get("facts") or {}); matched_variant = variant.get("id"); break
    if matched_variant: facts["variant"] = matched_variant
...
def gpu_requirements(part: dict[str, Any], exact_facts: dict[str, Any] | None = None) -> dict[str, Any]:
    requirements = dict(part.get("compatibility_requirements") or {}); text = str(part.get("host_requirements") or "")
    ...
    if explicit_psu is not None: requirements.setdefault("minimum_psu_w", explicit_psu)
    elif watt_values: requirements.setdefault("minimum_psu_w", max(watt_values))
    requirements.setdefault("gpu_length_mm", part.get("length_mm")); requirements.setdefault("gpu_slots", part.get("slot_width"))
    ...
    if exact_facts:
        ...
        for key in ("pcie_slot", "minimum_pcie_lanes", "minimum_pcie_generation", "minimum_psu_w", "gpu_length_mm", "gpu_slots", "power_connectors", "requires_resizable_bar"):
            if exact_facts.get(key) is not None: requirements[key] = exact_facts[key]
...
def evaluate_build_compatibility(build: dict[str, dict[str, Any]], gpu_part: dict[str, Any] | None = None, gpu_facts: dict[str, Any] | None = None) -> dict[str, Any]:
    failures: list[str] = []; unknowns: list[str] = []
    cpu = build.get("cpu_host", {}).get("compatibility_facts") or {}; board = build.get("motherboard", {}).get("compatibility_facts") or {}; ram = build.get("host_ram_32gb", {}).get("compatibility_facts") or {}; storage = build.get("storage_1tb", {}).get("compatibility_facts") or {}; psu = build.get("psu_750w", {}).get("compatibility_facts") or {}; chassis = build.get("chassis", {}).get("compatibility_facts") or {}; cooler = build.get("cooling", {}).get("compatibility_facts") or {}
    _check_equal("cpu_socket", cpu.get("socket"), board.get("socket"), failures, unknowns)
    cpu_memory = set(str(v).upper() for v in cpu.get("memory_types", [])); board_memory = str(board.get("memory_type") or "").upper() or None; ram_memory = str(ram.get("memory_type") or "").upper() or None
    ...
    board_forms = set(str(v).upper() for v in board.get("form_factors", [])); chassis_forms = set(str(v).upper() for v in chassis.get("motherboard_form_factors", []))
    ...
    if gpu_part:
        gpu = gpu_requirements(gpu_part, gpu_facts); board_slot = str(board.get("gpu_slot") or "") or None
        ...
        lane_req = int(gpu.get("minimum_pcie_lanes") or 0); lanes = board.get("gpu_slot_lanes")
        ...
        gen_req = int(gpu.get("minimum_pcie_generation") or 0); board_gen = board.get("pcie_generation")
        if gen_req and board_gen is not None and int(board_gen) < gen_req: failures.append(f"pcie_generation: Gen{board_gen} < required Gen{gen_req}")
        elif gen_req and board_gen is None: unknowns.append("pcie_generation")
        ...
        required_connectors = set(str(v) for v in gpu.get("power_connectors", []) if v); available_connectors = set(str(v) for v in psu.get("gpu_power_connectors", []) if v)
        ...
        if gpu.get("requires_resizable_bar") is True:
            if board.get("supports_resizable_bar") is False: failures.append("resizable_bar: GPU requires Resizable BAR but motherboard is explicitly unsupported")
            elif board.get("supports_resizable_bar") is not True: unknowns.append("resizable_bar_support")
    ...
    return {"status": status, "confidence": confidence, "failures": failures, "unknowns": sorted(set(unknowns)), "gpu_requirement_basis": "exact_sku_manufacturer_spec" if gpu_facts else "catalog_reference"}


def construct_compatible_builds(components: dict[str, Any], *, gpu_part: dict[str, Any] | None = None, gpu_facts: dict[str, Any] | None = None, required_components: list[str] | None = None, max_candidates_per_component: int = 5, maximum_builds: int = 25, allow_provisional: bool = True) -> list[dict[str, Any]]:
    required = required_components or ["cpu_host", "motherboard", "host_ram_32gb", "storage_1tb", "psu_750w", "cooling", "chassis"]; pools: list[list[dict[str, Any]]] = []
    ...
    for combination in itertools.product(*pools):
        selected = {component: row for component, row in zip(required, combination, strict=True)}; compatibility = evaluate_build_compatibility(selected, gpu_part, gpu_facts)
        ...
        component_cost = sum(float((row.get("landed") or {}).get("landed_cad") or 0.0) for row in selected.values()); confidence_penalty = len(compatibility["unknowns"]) * 5.0
    ...
    builds.sort(key=lambda row: (0 if row["compatibility"]["status"] == "compatible" else 1, row["ranking_cost_cad"], row["component_landed_cad"])); return builds[:maximum_builds]


def load_builds(path: Path | None = None) -> dict[str, Any]:
    target = path or project_root() / "data" / "market" / "compatible-builds.json"; return _load(target, {"schema_version": 2, "gpus": {}})

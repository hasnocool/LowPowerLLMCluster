...
from .spec_enrichment import enrich_bom_candidates
...
def _recent_gpu_listing(part_id: str, fx: dict[str, float], *, tax_rate: float, history_path: Path | None = None, max_age_hours: float = 168.0) -> dict[str, Any] | None:
    ...
    for row in history.get("observations", []):
        ...
        candidates.append({"listing": asdict(listing), "landed": landed, "configuration_confidence": row.get("configuration_confidence"), "seller_confidence": row.get("seller_confidence")})
    if not candidates: return None
...
def _write_compatible_builds(output: dict[str, Any], config: dict[str, Any], fx: dict[str, float], *, tax_rate: float, path: Path | None = None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"schema_version": 2, "generated_at": output["generated_at"], "currency": "CAD", "spec_enrichment_generated_at": output.get("spec_enrichment_generated_at"), "gpus": {}}
    gpu_parts = [part for part in load_catalog()["parts"] if part.get("category") == "gpu_accelerator"]
    for gpu in gpu_parts:
        gpu_id = str(gpu["id"]); gpu_market = _recent_gpu_listing(gpu_id, fx, tax_rate=tax_rate)
        builds = construct_compatible_builds(output["components"], gpu_part=gpu, required_components=list(solver.get("required_components") or []), max_candidates_per_component=int(selection.get("max_candidates_per_component_for_build_solver", 5)), maximum_builds=int(selection.get("maximum_compatible_builds", 25)), allow_provisional=bool(selection.get("allow_provisional_compatibility", True)))
        ...
        for build in builds:
            ...
            enriched_fields = sum(len((row.get("compatibility_fact_provenance") or {})) for row in build.get("components", {}).values())
            build["manufacturer_spec_fields"] = enriched_fields
        ...
        result["gpus"][gpu_id] = {"name": gpu.get("name"), "gpu_market": gpu_market, "build_count": len(builds), "best_build": builds[0] if builds else None, "builds": builds}
...
async def refresh_bom_market(*, config_path: Path | None = None, output_path: Path | None = None, history_path: Path | None = None, builds_path: Path | None = None, spec_config_path: Path | None = None, spec_evidence_path: Path | None = None) -> dict[str, Any]:
    ...
    output: dict[str, Any] = {"schema_version": 3, "generated_at": _now(), "currency": "CAD", "components": {}, "source_status": []}
    history_target = history_path or project_root() / "data" / "market" / "bom-price-history.json"
    history = _load(history_target, {"schema_version": 1, "observations": []}); observations = history.setdefault("observations", [])
    ...
    output["spec_enrichment_generated_at"] = enrichment.get("generated_at")
    output["spec_enriched_candidate_count"] = len(enrichment.get("records", []))

    for component, component_row in output["components"].items():
        # Re-select after enrichment so the selected object is always drawn from the enriched candidate pool.
        component_row["selected"] = select_bom_candidate(component_row.get("candidates") or [], config)
        selected = component_row.get("selected")
...
def sourced_component_costs(path: Path | None = None) -> tuple[dict[str, float], dict[str, dict[str, Any]]]:
    target = path or project_root() / "data" / "market" / "bom-current.json"; payload = _load(target, {"components": {}}); costs: dict[str, float] = {}; evidence: dict[str, dict[str, Any]] = {}

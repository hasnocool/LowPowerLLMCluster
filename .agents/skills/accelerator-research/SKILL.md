...
4. Find current model/runtime evidence. A TOPS number without a compiler/runtime path is not enough to mark `llm_candidate=true`, but it may still be a valid research/watch catalog item.
5. Record host requirements. M.2/PCIe/USB accelerators are not complete nodes; buying-cost estimates should account for the host even when no benchmark exists.
...
Catalog/watch inclusion requires credible identity/spec/source information, not local ownership. Strong purchase recommendations should become more cautious as performance evidence, current pricing, software maturity or host-cost data become weaker. Unknown is allowed.

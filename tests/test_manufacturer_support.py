...
@pytest.mark.asyncio
async def test_paginated_json_matrix_is_complete_only_after_all_pages():
    pages = {
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        result = await ingest_support_endpoint("https://www.msi.com/support/cpu?page=1", client=client, expected_host="msi.com")
...
@pytest.mark.asyncio
async def test_plain_html_matrix_remains_partial_without_explicit_proof():
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        result = await ingest_support_endpoint("https://www.gigabyte.com/support/cpu", client=client, expected_host="gigabyte.com")
    assert result["row_count"] == 1
    assert result["complete"] is False


@pytest.mark.asyncio
async def test_official_host_boundary_is_enforced():
    async with httpx.AsyncClient(transport=httpx.MockTransport(lambda request: httpx.Response(200, json={}, request=request))) as client:
        result = await ingest_support_endpoint("https://example.net/cpu", client=client, expected_host="asus.com")

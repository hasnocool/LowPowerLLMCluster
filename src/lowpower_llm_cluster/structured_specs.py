...
from .structured_identity import extract_structured_identity
...
def _merge(target: dict[str, Any], evidence: dict[str, Any], source: dict[str, Any], provenance: dict[str, Any]) -> None:
    for field, value in source.items():
        if value is None or field in target:
            continue
        target[field] = value
        evidence[field] = {"value": value, **provenance}
...
def _identity_pairs(documents: list[Any], tables: list[list[list[str]]]) -> list[tuple[str, str]]:
    ...
    for key, value in pairs:
        ...
        if component == "motherboard":
            ...
            if "connector" in label or "cable" in label:
                ...
                if any(term in lower for term in ("8 pin", "6+2")):
...
def _support_status(value, bios):
    ...
    if any(term in normalized for term in ("unsupported", "not supported", "not support", "no support")) or normalized in {"no", "n", "false"}:
        return "unsupported"
    if any(term in normalized for term in ("supported", "support", "validated", "yes")) or (bios and _norm(bios) not in {"-", "n/a", "na", "none", "not supported"}):
        return "supported"
    return "unknown"
...
def extract_cpu_support_rows(tables):
    ...
    for ti, table in enumerate(tables):
        ...
        cpu_idx = next((i for i, c in enumerate(headers) if any(t in c for t in ("cpu", "processor", "model"))), None)
        bios_idx = next((i for i, c in enumerate(headers) if "bios" in c), None)
        status_idx = next((i for i, c in enumerate(headers) if any(t in c for t in ("support", "status", "validated"))), None)
        if cpu_idx is None or bios_idx is None:
            continue
        for ri, row in enumerate(table[1:], start=1):
            if len(row) <= max(cpu_idx, bios_idx):
                continue
            cpu = row[cpu_idx].strip()
            bios = row[bios_idx].strip()
            if cpu:
                rows.append({"cpu_model": cpu, "minimum_bios_version": bios or None, "support_status": _support_status(row[status_idx].strip() if status_idx is not None and len(row) > status_idx else "", bios), "table_index": ti, "row_index": ri})
    return rows
...
def extract_cpu_support_matrix(tables, target_cpu=None):
    ...
    for row in extract_cpu_support_rows(tables):
        if target and target not in _norm(row.get("cpu_model")):
            continue
...
async def ingest_structured_manufacturer_document(component, html, source_url, observed_at, association_id, identity_score, *, client=None, target_cpu=None, allowed_host=None):
    ...
    stats = {
        ...
        "unlinked_support": {"candidates": [], "attempts": [], "selected_endpoint": None},
        "bios_history": [],
        "revision_bios_history": [],
        "board_revisions": [],
        "shipped_bios": {"status": "unknown", "shipped_bios_version": None, "meets_minimum": None},
    ...
    _merge(facts, evidence, jsonld, {"source_url": source_url, "source_type": "manufacturer_structured", "observed_at": observed_at, "extraction": "schema_org_additionalProperty", "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
    stats["jsonld_fields"] = len(jsonld)
    table_facts, _ = extract_table_facts(component, parser.tables)
    _merge(facts, evidence, table_facts, {"source_url": source_url, "source_type": "manufacturer_structured", "observed_at": observed_at, "extraction": "html_spec_table", "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
    stats["table_fields"] = len(table_facts)

    identity_facts = extract_structured_identity(_identity_pairs(parser.jsonld, parser.tables))
    _merge(facts, evidence, identity_facts, {"source_url": source_url, "source_type": "manufacturer_structured_identity", "observed_at": observed_at, "extraction": "structured_identity_property", "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
    stats["identity_fields"] = len(identity_facts)
    ...
    if component == "motherboard":
        ...
        stats["board_revisions"] = extract_board_revision_evidence(html, source_url=source_url)
        support_rows = extract_cpu_support_rows(parser.tables)
        for row in support_rows:
            row.update({"source_url": source_url, "source_type": "manufacturer_support_table", "observed_at": observed_at, "extraction": "cpu_bios_support_matrix", "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
        ...
        if client is not None and source_host and stats["support_endpoints"]:
            api_result = await ingest_ranked_support_endpoints(stats["support_endpoints"], client=client, official_host=source_host)
            stats["support_api"] = {k: v for k, v in api_result.items() if k != "matrix"}
            api_rows = list(api_result.get("matrix") or [])
            for row in api_rows:
                row.update({"observed_at": observed_at, "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
        ...
        if client is not None and source_host and not stats["cpu_support_matrix_complete"]:
            discovery = await discover_unlinked_support_surfaces(source_url, html, client=client, official_host=source_host)
            probed = await probe_unlinked_support_candidates(discovery, client=client)
            stats["unlinked_support"] = {"candidates": discovery.get("candidates", []), "discovery_sources": discovery.get("discovery_sources", []), "attempts": probed.get("attempts", []), "selected_endpoint": probed.get("support_endpoint")}
            stats["bios_history"] = probed.get("bios_history", [])
            stats["revision_bios_history"] = probed.get("revision_bios_history", [])
            deep_rows = list(probed.get("support_matrix") or [])
            for row in deep_rows:
                row.update({"observed_at": observed_at, "association_id": association_id, "identity_score": identity_score, "confidence": confidence})
            if deep_rows and (probed.get("support_complete") or len(deep_rows) > len(stats["cpu_support_matrix"])):
                stats["cpu_support_matrix"] = deep_rows
                stats["cpu_support_matrix_complete"] = bool(probed.get("support_complete"))
                stats["cpu_support_matrix_completeness_proof"] = probed.get("support_completeness_proof")
        minimum = support.get("minimum_bios_version") if support else None
        stats["shipped_bios"] = shipped_bios_evidence(html, minimum_bios=minimum, source_url=source_url)

    ...
    if args.distributed_coordinator:
        ...
        if args.distributed_tls_ca or args.distributed_tls_cert or args.distributed_tls_insecure_skip_verify:
            ssl_context = await asyncio.to_thread(
                ...
                ca=args.distributed_tls_ca,
                cert=args.distributed_tls_cert,
                key=args.distributed_tls_key,
                insecure_skip_verify=args.distributed_tls_insecure_skip_verify,
            )
        async with SecureDistributedCycleEngine(
            args.config,
            coordinator=args.distributed_coordinator,
            admin_token=token,
            history_path=args.history,
            output_path=args.output,
            poll_s=args.distributed_poll_s,
            timeout_s=args.distributed_timeout_s,
...
def main() -> int:
    ...
    parser.add_argument("--interval", type=float, default=300.0); parser.add_argument("--cycles", type=int); parser.add_argument("--health-host", default="127.0.0.1"); parser.add_argument("--health-port", type=int, default=8787); parser.add_argument("--readiness-max-age", type=float, default=900.0)
    ...
    if min(args.interval, args.readiness_max_age, args.distributed_poll_s, args.distributed_timeout_s) <= 0 or args.health_port < 1:
        parser.error("interval, readiness, distributed timing and health-port values must be positive")
